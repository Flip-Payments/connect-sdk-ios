...
extension FCApi {
    static func getAddresses(accessToken token: String, completion: @escaping (_ addresses: AddressesResponse, _ error: Error?) -> Void) {
        ...
        var err: Error? = nil
        FCApi.request(toURL: URL(string: "\(FCConfiguration.environment.userManagementURL)user/account/addresses")!, withVerb: .get, withParameters: nil, withHeaders: headers) { response, error in
